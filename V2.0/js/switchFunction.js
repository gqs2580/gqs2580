...
function display_none_all() {
		...
		compareCity.style.display = "none"
		provinceData.style.display = "none"

		weatherGame.style.display = "none"
		bigLookContainer.style.display = "none"
		bigLook.style.display = "none"
...
function province_data() {
	...
	if (provinceData.children.length == 0)
		myChart = echarts.init(provinceData)
	convertData(data)
	...
	var option = {
		baseOption: {
			...
			backgroundColor: {
				...
				colorStops: [{ //渐变
						offset: 0,
						color: allProvinceGradientColor[0] // 0% 处的颜色
					},
					{
						offset: 1,
						color: allProvinceGradientColor[1] //100%处的颜色
					}
				],
			},
			...
			series: [{
					...
					tooltip: {
						...
						formatter: function(params) {
							return params.name + ' : ' + params.value[2] + '度';
						}
					},
				},
				{
				 ...
				 , {
					...
					symbol: `image://./img/dingwei.png`, //自定义的展示图标
					symbolSize: 40,
...
function compare_city() {
	...
	//添加城市下拉
	var compare_city_top = document.querySelector('.compare_city_top select')
	for (var i = 0; i < province.length; i++) {
		var option = document.createElement("Option")
		option.innerHTML = province[i]
		option.value = province[i]
		compare_city_top.appendChild(option)
	}
	compareCity.style.display = "block"
}

function weather_game() {
	weatherGame.style.display = "flex"
}

function big_look() {
	bigLook.style.display = "block"
	bigLookContainer.style.display = "block"
}